...
class Propagate:
    ...
    def __init__(self, propagator: PropSetup, sc: Spacecraft, stop_param: str, stop_value: str | int = None,
        ...
        if stop_param == 'ElapsedSecs':  # assuming stop_value is an ElapsedSecs value
            ...
            # self.dt = stop_value * 86400
        else:
            raise NotImplementedError

        # TODO use direction
        if isinstance(backProp, bool):
            # propagate backwards (direction -1) if backProp is True, otherwise forwards (direction 1)
            self.direction = -1 if backProp else 1
        else:
